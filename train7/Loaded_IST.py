...
def get_content_style():
    content_module = Content().to(device)
    content_module.load_state_dict(torch.load('content_module.pth'))
    style_module = Style().to(device)
    style_module.load_state_dict(torch.load('style_module.pth'))
...
def get_pic(style_path_in, content_path_in):
    ...
    height_in = int(height_in / SCALE)
    width_in = int(width_in / SCALE)
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        # create modules
        picture_module = Picture(height, width).to(device)
        picture_module.load_state_dict(torch.load('picture_module.pth'))
    ...
    for epoch in range(EPOCH):
        ...
        if epoch % 10 == 9:  # print every 10 mini-batches
            print('EPOCH: %5d  Loss: %.6f' % (epoch + 1, running_loss / 10))
            running_loss = 0.0
